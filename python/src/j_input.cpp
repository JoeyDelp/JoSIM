...
void input(py::module& m)
{
  ...
  py::class_<Parameter>(m, "Parameter")
      .def("has_unused_parameters", [](Parameter& param) {
        return !param.unparsedParams.empty();
      });
  ...
  py::class_<Input>(m, "Input")
      .def(py::init<>())
      .def("display_help", [](Input& input) {
        py::scoped_ostream_redirect cout;
        py::scoped_estream_redirect cerr;
        input.display_help();
      })
      .def("version_info", [](Input& input) {
        py::scoped_ostream_redirect cout;
        py::scoped_estream_redirect cerr;
        input.version_info();
      })
      .def("read_input_file", [](Input& input) {
        input.read_input_file(input.fileName, input.fileLines);
      })
      .def("split_netlist", [](Input& input) {
        input.split_netlist(input.fileLines, input.controls, input.parameters, input.netlist);
      })
      .def("parse_parameters", [](Input& input) {
        if (input.parameters.unparsedParams.size() > 0)
          Parser::parse_parameters(input.parameters.unparsedParams, input.parameters.parsedParams);
      })
      .def("expand_subcircuits", &Input::expand_subcircuits)
      .def("expand_maindesign", &Input::expand_maindesign)
      .def("parse_arguments", [](Input& input, std::vector<std::string> args) {
