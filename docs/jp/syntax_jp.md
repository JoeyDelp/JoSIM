...
***CPR***を設定すると、超電流位相関係の係数が変更されます。デフォルトは$I_S=I_C\sin(\phi)$に関連する1です。これを2に設定すると、この関係が$I_S = I_C\sin(2\phi)$に変更され、超電流の2次高調波のみを有効にして、最初の高調波を抑制することにより、$2\phi$-junctionを使用できるようになります。[^1][^2]
