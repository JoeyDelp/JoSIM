...
*/
void phase_traces_to_plot(InputFile& iFile, std::vector<std::string> controlPart, std::vector<std::string>& traceLabel, std::vector<std::vector<double>>& traceData) {
	std::vector<std::string> tokens, labeltokens, nodesTokens;
	std::vector<double> trace;
	std::map<std::string, std::vector<double>> traces;
	std::string columnLabel1, columnLabel2, label, nodesToPlot;
	...
	for (const auto &string : controlPart) {
		...
		if (string.find("PRINT") != std::string::npos) {
			...
			/*****************************************************************************************************/
				...
				switch(tokens[2][0]) {
					case 'B':
						if(iFile.pJJ.count(tokens[2]) != 0){
							trace.clear();
							if(iFile.pJJ[tokens[2]].pPR == -1) {
								trace = iFile.xVect[iFile.pJJ[tokens[2]].pNR];
								std::fill(trace.begin(), trace.end(), 0.0);
								std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pJJ[tokens[2]].pNR].begin(), trace.begin(), std::minus<double>());
							}
							else if(iFile.pJJ[tokens[2]].pNR == -1) trace = iFile.xVect[iFile.pJJ[tokens[2]].pPR];
							else {
								trace = iFile.xVect[iFile.pJJ[tokens[2]].pPR];
								std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pJJ[tokens[2]].pNR].begin(), trace.begin(), std::minus<double>());
							}
							traceLabel.push_back(label);
							traceData.push_back(trace);
						...
						break;
					case 'L':
						if(iFile.pInd.count(tokens[2]) != 0){
							trace.clear();
							if(iFile.pInd[tokens[2]].pPR == -1) {
								trace = iFile.xVect[iFile.pInd[tokens[2]].pNR];
								std::fill(trace.begin(), trace.end(), 0.0);
								std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pInd[tokens[2]].pNR].begin(), trace.begin(), std::minus<double>());
							}
							else if(iFile.pInd[tokens[2]].pNR == -1) trace = iFile.xVect[iFile.pInd[tokens[2]].pPR];
							else {
								trace = iFile.xVect[iFile.pInd[tokens[2]].pPR];
								std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pInd[tokens[2]].pNR].begin(), trace.begin(), std::minus<double>());
							}
							traceLabel.push_back(label);
							traceData.push_back(trace);
						...
						break;
					case 'R':
						if(iFile.pRes.count(tokens[2]) != 0){
							trace.clear();
							if(iFile.pRes[tokens[2]].pPR == -1) {
								trace = iFile.xVect[iFile.pRes[tokens[2]].pNR];
								std::fill(trace.begin(), trace.end(), 0.0);
								std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pRes[tokens[2]].pNR].begin(), trace.begin(), std::minus<double>());
							}
							else if(iFile.pRes[tokens[2]].pNR == -1) trace = iFile.xVect[iFile.pRes[tokens[2]].pPR];
							else {
								trace = iFile.xVect[iFile.pRes[tokens[2]].pPR];
								std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pRes[tokens[2]].pNR].begin(), trace.begin(), std::minus<double>());
							}
							traceLabel.push_back(label);
							traceData.push_back(trace);
						...
						break;
					case 'C':
						if(iFile.pCap.count(tokens[2]) != 0){
							trace.clear();
							if(iFile.pCap[tokens[2]].pPR == -1) {
								trace = iFile.xVect[iFile.pCap[tokens[2]].pNR];
								std::fill(trace.begin(), trace.end(), 0.0);
								std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pCap[tokens[2]].pNR].begin(), trace.begin(), std::minus<double>());
							}
							else if(iFile.pCap[tokens[2]].pNR == -1) trace = iFile.xVect[iFile.pCap[tokens[2]].pPR];
							else {
								trace = iFile.xVect[iFile.pCap[tokens[2]].pPR];
								std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pCap[tokens[2]].pNR].begin(), trace.begin(), std::minus<double>());
							}
							traceLabel.push_back(label);
							traceData.push_back(trace);
						...
						break;
					case 'V':
						if(iFile.pVS.count(tokens[2]) != 0){
							trace.clear();
							if(iFile.pVS[tokens[2]].pPR == -1) {
								trace = iFile.xVect[iFile.pVS[tokens[2]].pNR];
								std::fill(trace.begin(), trace.end(), 0.0);
								std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pVS[tokens[2]].pNR].begin(), trace.begin(), std::minus<double>());
							}
							else if(iFile.pVS[tokens[2]].pNR == -1) trace = iFile.xVect[iFile.pVS[tokens[2]].pPR];
							else {
								trace = iFile.xVect[iFile.pVS[tokens[2]].pPR];
								std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pVS[tokens[2]].pNR].begin(), trace.begin(), std::minus<double>());
							}
							traceLabel.push_back(label);
							traceData.push_back(trace);
				...
				switch(tokens[2][0]) {
					case 'B':
						if(iFile.pJJ.count(tokens[2]) != 0){
							trace.clear();
							if(iFile.pJJ[tokens[2]].pPR == -1) {
								Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * iFile.xVect[iFile.pJJ[tokens[2]].pNR][0];
								trace.push_back(Vn1);
								for (int i = 1; i < iFile.xVect[iFile.pJJ[tokens[2]].pNR].size(); i++) {
									Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * (iFile.xVect[iFile.pJJ[tokens[2]].pNR][i] -  iFile.xVect[iFile.pJJ[tokens[2]].pNR][i-1]) - Vn1;
									trace.push_back(Vn1);
								}
							}
							else if(iFile.pJJ[tokens[2]].pNR == -1) {
								Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * iFile.xVect[iFile.pJJ[tokens[2]].pPR][0];
								trace.push_back(Vn1);
								for (int i = 1; i < iFile.xVect[iFile.pJJ[tokens[2]].pPR].size(); i++) {
									Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * (iFile.xVect[iFile.pJJ[tokens[2]].pPR][i] -  iFile.xVect[iFile.pJJ[tokens[2]].pPR][i-1]) - Vn1;
									trace.push_back(Vn1);
								}
							}
							else {
								Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * (iFile.xVect[iFile.pJJ[tokens[2]].pPR][0] - iFile.xVect[iFile.pJJ[tokens[2]].pNR][0]);
								trace.push_back(Vn1);
								for (int i = 1; i < iFile.xVect[iFile.pJJ[tokens[2]].pPR].size(); i++) {
									Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * ((iFile.xVect[iFile.pJJ[tokens[2]].pPR][i] - iFile.xVect[iFile.pJJ[tokens[2]].pNR][i]) -  (iFile.xVect[iFile.pJJ[tokens[2]].pPR][i-1] - iFile.xVect[iFile.pJJ[tokens[2]].pNR][i-1])) - Vn1;
									trace.push_back(Vn1);
								}
							}
							traceLabel.push_back(label);
					...
					case 'L':
						if(iFile.pInd.count(tokens[2]) != 0){
							trace.clear();
							if(iFile.pInd[tokens[2]].pPR == -1) {
								Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * iFile.xVect[iFile.pInd[tokens[2]].pNR][0];
								trace.push_back(Vn1);
								for (int i = 1; i < iFile.xVect[iFile.pInd[tokens[2]].pNR].size(); i++) {
									Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * (iFile.xVect[iFile.pInd[tokens[2]].pNR][i] -  iFile.xVect[iFile.pInd[tokens[2]].pNR][i-1]) - Vn1;
									trace.push_back(Vn1);
								}
							}
							else if(iFile.pInd[tokens[2]].pNR == -1) {
								Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * iFile.xVect[iFile.pInd[tokens[2]].pPR][0];
								trace.push_back(Vn1);
								for (int i = 1; i < iFile.xVect[iFile.pInd[tokens[2]].pPR].size(); i++) {
									Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * (iFile.xVect[iFile.pInd[tokens[2]].pPR][i] -  iFile.xVect[iFile.pInd[tokens[2]].pPR][i-1]) - Vn1;
									trace.push_back(Vn1);
								}
							}
							else {
								Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * (iFile.xVect[iFile.pInd[tokens[2]].pPR][0] - iFile.xVect[iFile.pInd[tokens[2]].pNR][0]);
								trace.push_back(Vn1);
								for (int i = 1; i < iFile.xVect[iFile.pInd[tokens[2]].pPR].size(); i++) {
									Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * ((iFile.xVect[iFile.pInd[tokens[2]].pPR][i] - iFile.xVect[iFile.pInd[tokens[2]].pNR][i]) -  (iFile.xVect[iFile.pInd[tokens[2]].pPR][i-1] - iFile.xVect[iFile.pInd[tokens[2]].pNR][i-1])) - Vn1;
									trace.push_back(Vn1);
								}
							}
							traceLabel.push_back(label);
					...
					case 'R':
						if(iFile.pRes.count(tokens[2]) != 0){
							trace.clear();
							if(iFile.pRes[tokens[2]].pPR == -1) {
								Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * iFile.xVect[iFile.pRes[tokens[2]].pNR][0];
								trace.push_back(Vn1);
								for (int i = 1; i < iFile.xVect[iFile.pRes[tokens[2]].pNR].size(); i++) {
									Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * (iFile.xVect[iFile.pRes[tokens[2]].pNR][i] -  iFile.xVect[iFile.pRes[tokens[2]].pNR][i-1]) - Vn1;
									trace.push_back(Vn1);
								}
							}
							else if(iFile.pRes[tokens[2]].pNR == -1) {
								Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * iFile.xVect[iFile.pRes[tokens[2]].pPR][0];
								trace.push_back(Vn1);
								for (int i = 1; i < iFile.xVect[iFile.pRes[tokens[2]].pPR].size(); i++) {
									Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * (iFile.xVect[iFile.pRes[tokens[2]].pPR][i] -  iFile.xVect[iFile.pRes[tokens[2]].pPR][i-1]) - Vn1;
									trace.push_back(Vn1);
								}
							}
							else {
								Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * (iFile.xVect[iFile.pRes[tokens[2]].pPR][0] - iFile.xVect[iFile.pRes[tokens[2]].pNR][0]);
								trace.push_back(Vn1);
								for (int i = 1; i < iFile.xVect[iFile.pRes[tokens[2]].pPR].size(); i++) {
									Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * ((iFile.xVect[iFile.pRes[tokens[2]].pPR][i] - iFile.xVect[iFile.pRes[tokens[2]].pNR][i]) -  (iFile.xVect[iFile.pRes[tokens[2]].pPR][i-1] - iFile.xVect[iFile.pRes[tokens[2]].pNR][i-1])) - Vn1;
									trace.push_back(Vn1);
								}
							}
							traceLabel.push_back(label);
					...
					case 'C':
						if(iFile.pCap.count(tokens[2]) != 0){
							trace.clear();
							if(iFile.pCap[tokens[2]].pPR == -1) {
								Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * iFile.xVect[iFile.pCap[tokens[2]].pNR][0];
								trace.push_back(Vn1);
								for (int i = 1; i < iFile.xVect[iFile.pCap[tokens[2]].pNR].size(); i++) {
									Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * (iFile.xVect[iFile.pCap[tokens[2]].pNR][i] -  iFile.xVect[iFile.pCap[tokens[2]].pNR][i-1]) - Vn1;
									trace.push_back(Vn1);
								}
							}
							else if(iFile.pCap[tokens[2]].pNR == -1) {
								Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * iFile.xVect[iFile.pCap[tokens[2]].pPR][0];
								trace.push_back(Vn1);
								for (int i = 1; i < iFile.xVect[iFile.pCap[tokens[2]].pPR].size(); i++) {
									Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * (iFile.xVect[iFile.pCap[tokens[2]].pPR][i] -  iFile.xVect[iFile.pCap[tokens[2]].pPR][i-1]) - Vn1;
									trace.push_back(Vn1);
								}
							}
							else {
								Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * (iFile.xVect[iFile.pCap[tokens[2]].pPR][0] - iFile.xVect[iFile.pCap[tokens[2]].pNR][0]);
								trace.push_back(Vn1);
								for (int i = 1; i < iFile.xVect[iFile.pCap[tokens[2]].pPR].size(); i++) {
									Vn1 = (PHI_ZERO) / (M_PI * iFile.tsim.prstep) * ((iFile.xVect[iFile.pCap[tokens[2]].pPR][i] - iFile.xVect[iFile.pCap[tokens[2]].pNR][i]) -  (iFile.xVect[iFile.pCap[tokens[2]].pPR][i-1] - iFile.xVect[iFile.pCap[tokens[2]].pNR][i-1])) - Vn1;
									trace.push_back(Vn1);
								}
							}
							traceLabel.push_back(label);
			...
			/*****************************************************************************************************/
				...
				switch(tokens[2][0]) {
					case 'B':
						if(iFile.pJJ.count(tokens[2]) != 0){
							trace.clear();
							trace = iFile.xVect[iFile.pJJ[tokens[2]].bIRR];
							std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pJJ[tokens[2]].bICR].begin(), trace.begin(), std::plus<double>());
							std::transform(trace.begin(), trace.end(), iFile.pJJ[tokens[2]].junctionCurrent.begin(), trace.begin(), std::plus<double>());
							traceLabel.push_back(label);
							traceData.push_back(trace);
						}
						else {
					...
					case 'L':
						if(iFile.pInd.count(tokens[2]) != 0){
							trace.clear();
							trace = iFile.xVect[iFile.pInd[tokens[2]].bIR];
							traceLabel.push_back(label);
							traceData.push_back(trace);
						}
						else {
					...
					case 'R':
						if(iFile.pRes.count(tokens[2]) != 0){
							trace.clear();
							trace = iFile.xVect[iFile.pRes[tokens[2]].bIR];
							traceLabel.push_back(label);
							traceData.push_back(trace);
						}
						else {
					...
					case 'C':
						if(iFile.pCap.count(tokens[2]) != 0){
							trace.clear();
							trace = iFile.xVect[iFile.pCap[tokens[2]].bIR];
							traceLabel.push_back(label);
							traceData.push_back(trace);
						}
						else {
					...
					case 'V':
						if(iFile.pVS.count(tokens[2]) != 0){
							trace.clear();
							trace = iFile.xVect[iFile.pVS[tokens[2]].bIR];
							traceLabel.push_back(label);
							traceData.push_back(trace);
						}
						else {
		...
		/****************************************************/
			...
			for (int k = 1; k < tokens.size(); k++) {
				...
				if (tokens[k][0] == 'V') {
					...
					case 'B':
						if(iFile.pJJ.count(nodesToPlot) != 0){
							trace.clear();
							trace = iFile.xVect[iFile.pJJ[nodesToPlot].bIRR];
							std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pJJ[nodesToPlot].bICR].begin(), trace.begin(), std::plus<double>());
							std::transform(trace.begin(), trace.end(), iFile.pJJ[nodesToPlot].junctionCurrent.begin(), trace.begin(), std::plus<double>());
							traceLabel.push_back(label);
							traceData.push_back(trace);
						}
						else {
						...
						break;
					case 'L':
						if(iFile.pInd.count(nodesToPlot) != 0){
							trace.clear();
							trace = iFile.xVect[iFile.pInd[nodesToPlot].bIR];
							traceLabel.push_back(label);
							traceData.push_back(trace);
						}
						else {
						...
						break;
					case 'R':
						if(iFile.pRes.count(nodesToPlot) != 0){
							trace.clear();
							trace = iFile.xVect[iFile.pRes[nodesToPlot].bIR];
							traceLabel.push_back(label);
							traceData.push_back(trace);
						}
						else {
						...
						break;
					case 'C':
						if(iFile.pCap.count(nodesToPlot) != 0){
							trace.clear();
							trace = iFile.xVect[iFile.pCap[nodesToPlot].bIR];
							traceLabel.push_back(label);
							traceData.push_back(trace);
						}
						else {
						...
						break;
					case 'V':
						if(iFile.pVS.count(nodesToPlot) != 0){
							trace.clear();
							trace = iFile.xVect[iFile.pVS[nodesToPlot].bIR];
							traceLabel.push_back(label);
							traceData.push_back(trace);
						}
						else {
					...
					switch(nodesToPlot[0]) {
						case 'B':
							if(iFile.pJJ.count(nodesToPlot) != 0){
								trace.clear();
								if(iFile.pJJ[nodesToPlot].pPR == -1) {
									trace = iFile.xVect[iFile.pJJ[nodesToPlot].pNR];
									std::fill(trace.begin(), trace.end(), 0.0);
									std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pJJ[nodesToPlot].pNR].begin(), trace.begin(), std::minus<double>());
								}
								else if(iFile.pJJ[nodesToPlot].pNR == -1) trace = iFile.xVect[iFile.pJJ[nodesToPlot].pPR];
								else {
									trace = iFile.xVect[iFile.pJJ[nodesToPlot].pPR];
									std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pJJ[nodesToPlot].pNR].begin(), trace.begin(), std::minus<double>());
								}
								traceLabel.push_back(label);
								traceData.push_back(trace);
						...
						case 'L':
							if(iFile.pInd.count(nodesToPlot) != 0){
								trace.clear();
								if(iFile.pInd[nodesToPlot].pPR == -1) {
									trace = iFile.xVect[iFile.pInd[nodesToPlot].pNR];
									std::fill(trace.begin(), trace.end(), 0.0);
									std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pInd[nodesToPlot].pNR].begin(), trace.begin(), std::minus<double>());
								}
								else if(iFile.pInd[nodesToPlot].pNR == -1) trace = iFile.xVect[iFile.pInd[nodesToPlot].pPR];
								else {
									trace = iFile.xVect[iFile.pInd[nodesToPlot].pPR];
									std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pInd[nodesToPlot].pNR].begin(), trace.begin(), std::minus<double>());
								}
								traceLabel.push_back(label);
								traceData.push_back(trace);
						...
						case 'R':
							if(iFile.pRes.count(nodesToPlot) != 0){
								trace.clear();
								if(iFile.pRes[nodesToPlot].pPR == -1) {
									trace = iFile.xVect[iFile.pRes[nodesToPlot].pNR];
									std::fill(trace.begin(), trace.end(), 0.0);
									std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pRes[nodesToPlot].pNR].begin(), trace.begin(), std::minus<double>());
								}
								else if(iFile.pRes[nodesToPlot].pNR == -1) trace = iFile.xVect[iFile.pRes[nodesToPlot].pPR];
								else {
									trace = iFile.xVect[iFile.pRes[nodesToPlot].pPR];
									std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pRes[nodesToPlot].pNR].begin(), trace.begin(), std::minus<double>());
								}
								traceLabel.push_back(label);
								traceData.push_back(trace);
						...
						case 'C':
							if(iFile.pCap.count(nodesToPlot) != 0){
								trace.clear();
								if(iFile.pCap[nodesToPlot].pPR == -1) {
									trace = iFile.xVect[iFile.pCap[nodesToPlot].pNR];
									std::fill(trace.begin(), trace.end(), 0.0);
									std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pCap[nodesToPlot].pNR].begin(), trace.begin(), std::minus<double>());
								}
								else if(iFile.pCap[nodesToPlot].pNR == -1) trace = iFile.xVect[iFile.pCap[nodesToPlot].pPR];
								else {
									trace = iFile.xVect[iFile.pCap[nodesToPlot].pPR];
									std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pCap[nodesToPlot].pNR].begin(), trace.begin(), std::minus<double>());
								}
								traceLabel.push_back(label);
								traceData.push_back(trace);
						...
						case 'V':
							if(iFile.pVS.count(nodesToPlot) != 0){
								trace.clear();
								if(iFile.pVS[nodesToPlot].pPR == -1) {
									trace = iFile.xVect[iFile.pVS[nodesToPlot].pNR];
									std::fill(trace.begin(), trace.end(), 0.0);
									std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pVS[nodesToPlot].pNR].begin(), trace.begin(), std::minus<double>());
								}
								else if(iFile.pVS[nodesToPlot].pNR == -1) trace = iFile.xVect[iFile.pVS[nodesToPlot].pPR];
								else {
									trace = iFile.xVect[iFile.pVS[nodesToPlot].pPR];
									std::transform(trace.begin(), trace.end(), iFile.xVect[iFile.pVS[nodesToPlot].pNR].begin(), trace.begin(), std::minus<double>());
								}
								traceLabel.push_back(label);
								traceData.push_back(trace);
...
int plot_all_traces(InputFile& iFile) {
		...
		if (iFile.matA.columnNames.size() <= 3) {
			...
			for (auto i : iFile.matA.columnNames) {
				...
				if (substring_after(i, "C_")[0] == 'N'){
					if (substring_after(i, "C_")[1] == 'P') plt::ylabel("Phase (rads)");
					else if (substring_after(i, "C_")[1] == 'V') plt::ylabel("Voltage (V)");
				}
			...
			for (int j = 0; j < iFile.matA.columnNames.size(); j = j + 3) {
				...
				while ((counter < iFile.matA.columnNames.size()) && (counter < j + 3)) {
					...
					if (substring_after(iFile.matA.columnNames[counter], "C_")[0] == 'N'){
						if (substring_after(iFile.matA.columnNames[counter], "C_")[1] == 'V') plt::ylabel("Voltage (V)");
						else if (substring_after(iFile.matA.columnNames[counter], "C_")[1] == 'P') plt::ylabel("Phase (rads)");
					}
