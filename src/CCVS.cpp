...
CCVS::CCVS(const std::pair<tokens_t, string_o>& s,
...
void CCVS::set_node_indices(const tokens_t& t, const nodemap& nm, nodeconnections& nc) {
    ...
    switch (nodeConfig2_) {
        case NodeConfig::POSGND:
            ...
            nc.at(nm.at(t.at(2))).emplace_back(std::make_pair(1, indexInfo.currentIndex_.value()));
            break;
        case NodeConfig::GNDNEG:
            negIndex2_ = nm.at(t.at(3));
            nc.at(nm.at(t.at(3))).emplace_back(std::make_pair(-1, indexInfo.currentIndex_.value()));
            break;
        case NodeConfig::POSNEG:
            posIndex2_ = nm.at(t.at(2));
            negIndex2_ = nm.at(t.at(3));
...
void CCVS::set_matrix_info(const AnalysisType& at, const double& h) {
    ...
    if (at == AnalysisType::Voltage) {
        ...
        pn2_ = 0;
        matrixInfo.nonZeros_.emplace_back(-netlistInfo.value_ * (1.0 / Constants::SIGMA) * ((2 * h) / 3.0));
...
void CCVS::update_timestep(const double& factor) {
    if (at_ == AnalysisType::Phase) { matrixInfo.nonZeros_.at(hDepPos_) = factor * matrixInfo.nonZeros_.at(hDepPos_); }
}
