...
/*
  ...
  (PHASE)
  phi - (2e/hbar)(2h/3G)Io = (4/3)phin-1 - (1/3)phin-2

  [ 0  0  0  0                 1 ] [ phio+ ]   [                       0 ]
  | 0  0  0  0                -1 | | phio- |   |                       0 |
  | 0  0  0  0                 0 | | phic+ | = |                       0 |
  | 0  0  0  0                 0 | | phic- |   |                       0 |
  [ 0  0  1 -1 -(2e/hbar)(2h/3G) ] [  Io   ]   [ (4/3)phin-1-(1/3)phin-2 ]
*/

VCCS::VCCS(const std::pair<tokens_t, string_o>& s,
    ...
    if (at == AnalysisType::Voltage) {
        ...
        pn2_ = 0;
        // Append the value to the non zero vector
        matrixInfo.nonZeros_.emplace_back(-(1.0 / Constants::SIGMA) * ((2.0 * h) / (3 * netlistInfo.value_)));
    }
}

void VCCS::set_node_indices(const tokens_t& t, const nodemap& nm, nodeconnections& nc) {
    ...
    switch (indexInfo.nodeConfig_) {
        case NodeConfig::POSGND:
            ...
            nc.at(nm.at(t.at(0))).emplace_back(std::make_pair(1, indexInfo.currentIndex_.value()));
            break;
        case NodeConfig::GNDNEG:
            indexInfo.negIndex_ = nm.at(t.at(1));
            nc.at(nm.at(t.at(1))).emplace_back(std::make_pair(-1, indexInfo.currentIndex_.value()));
            break;
    ...
    switch (nodeConfig2_) {
        case NodeConfig::POSGND: posIndex2_ = nm.at(t.at(2)); break;
        case NodeConfig::GNDNEG: negIndex2_ = nm.at(t.at(3)); break;
        case NodeConfig::POSNEG:
...
void VCCS::update_timestep(const double& factor) {
    if (at_ == AnalysisType::Phase) { matrixInfo.nonZeros_.back() = factor * matrixInfo.nonZeros_.back(); }
}
