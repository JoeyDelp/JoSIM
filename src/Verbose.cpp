...
void Verbose::handle_verbosity(const int64_t& vl, const Input& iObj, const Matrix& mObj) {
    switch (vl) {
        case 0: break;
        case 1: print_circuit_stats(iObj, mObj); break;
        case 2:
        ...
        case 3:
            ...
            print_circuit_stats(iObj, mObj);
            break;
        default: Errors::verbosity_errors(VerbosityErrors::NO_SUCH_LEVEL, std::to_string(vl)); break;
    }
}
...
void Verbose::print_circuit_stats(const Input& iObj, const Matrix& mObj) {
    ...
    if (mObj.components.mutualinductances.size() != 0) {
        std::cout << std::left << std::setw(26)
                  << "Mutual inductance count:" << mObj.components.mutualinductances.size() << "\n";
    }
...
void Verbose::print_parameters(const Input& iObj) {
    if (iObj.parameters.size() != 0) {
        ...
        for (auto i : iObj.parameters) {
            ...
            value << std::scientific << std::setprecision(6) << i.second.get_value().value();
            sortedParams.emplace_back(i.first.subcircuit().value_or("") + " : " + i.first.name() + " = " + value.str());
        }
        std::sort(sortedParams.begin(), sortedParams.end());
        for (auto i : sortedParams) { std::cout << i << std::endl; }
        std::cout << std::endl;
    }
}
...
void Verbose::print_expanded_netlist(const Input& iObj) {
    ...
    for (const auto& i : iObj.netlist.expNetlist) { std::cout << Misc::vector_to_string(i.first) << std::endl; }
    std::cout << std::endl;
}
