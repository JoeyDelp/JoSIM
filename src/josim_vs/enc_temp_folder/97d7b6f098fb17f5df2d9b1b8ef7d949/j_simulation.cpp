	...
	for (int i = 0; i < tsim.simsize(); i++) {
		...
		for (auto j : rowNames) {
			...
			if (j.find("_N") != std::string::npos) {
				...
				if (VP == -1.0) RHSvalue = initialConditionsMap["R_" + currentLabel + "-P_PREV"] + (((tsim.maxtstep / 2)*(2 * M_PI / PHI_ZERO))*(-lhsValues[(int)VN]));
				else if (VN == -1.0) RHSvalue = initialConditionsMap["R_" + currentLabel + "-P_PREV"] + (((tsim.maxtstep / 2)*(2 * M_PI / PHI_ZERO))*(lhsValues[(int)VP]));
				else RHSvalue = initialConditionsMap["R_" + currentLabel + "-P_PREV"] + (((tsim.maxtstep / 2)*(2 * M_PI / PHI_ZERO))*(lhsValues[(int)VP] - lhsValues[(int)VN]));
				RHS.push_back(RHSvalue);
			}
