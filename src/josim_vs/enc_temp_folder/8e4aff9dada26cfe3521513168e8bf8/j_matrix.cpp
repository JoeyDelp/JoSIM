...
void create_A_matrix(InputFile& iFile) {
	...
	for (auto i : iFile.maincircuitSegment) {
		...
		try {
			...
			else invalid_component_errors(DUPLICATE_LABEL, label);
		}
		catch (const std::out_of_range) {
		...
		if (i[0] == 'R') {
			...
			bMatrixConductanceMap[rNameP][label + "-VP"] = -1.0;
			bMatrixConductanceMap[rNameP][label + "-VN"] = -1.0;
			bMatrixConductanceMap[rNameN][label + "-VP"] = -1.0;
			bMatrixConductanceMap[rNameN][label + "-VN"] = -1.0;
