...
std::map<std::string, std::map<std::string, std::string>> bMatrixConductanceMap;
	...
	for (auto i : iFile.maincircuitSegment) {
		...
		if (i[0] == 'R') {
			...
			if (nodeP != "0" && nodeP.find("GND") == std::string::npos) {
				...
				bMatrixConductanceMap[rNameP][label] = std::to_string(0);
				pGND = false;
			}
			else pGND = true;
			if (nodeN != "0" && nodeN.find("GND") == std::string::npos) {
				...
				bMatrixConductanceMap[rNameN][label] = std::to_string(0);
				nGND = false;
			}
			else nGND = true;
			matrix_element e;
			...
			if (nodeP != "0" && nodeP.find("GND") == std::string::npos) {
				...
				unique_push(rowNames, rNameP);
				unique_push(columnNames, cNameP);
				bMatrixConductanceMap[rNameP][label] = std::to_string(value);
				pGND = false;
			}
			else pGND = true;
			if (nodeN != "0" && nodeN.find("GND") == std::string::npos) {
				...
				unique_push(rowNames, rNameN);
				unique_push(columnNames, cNameN);
				bMatrixConductanceMap[rNameN][label] = std::to_string(-value);
				nGND = false;
			}
			else nGND = true;
			if (!nGND) bMatrixConductanceMap[rNameP][label + "-V"] = cNameP + "-GND";
			else if (!pGND) bMatrixConductanceMap[rNameN][label + "-V"] = "GND-" + cNameN;
			else {
				bMatrixConductanceMap[rNameP][label + "-V"] = cNameP + "-" + cNameN;
				bMatrixConductanceMap[rNameN][label + "-V"] = cNameP + "-" + cNameN;
			}
			matrix_element e;
			if (!pGND) {
				/* Positive node row and column */
			...
			if (!nGND) bMatrixConductanceMap[rName][label + "-V"] = cNameP + "-GND";
			else if (!pGND)	bMatrixConductanceMap[rName][label + "-V"] = "GND-" + cNameN;
			else bMatrixConductanceMap[rName][label + "-V"] = cNameP + "-" + cNameN;
			bMatrixConductanceMap[rName][label + "-I"] = cName;
			matrix_element e;
			if (!pGND) {
				/* Positive node row and column */
				e.label = label;
			...
			unique_push(rowNames, rName);
			unique_push(columnNames, cName);
			bMatrixConductanceMap[rName][label] = label;
			if (nodeP != "0" && nodeP.find("GND") == std::string::npos) {
				cNameP = "C_NV" + nodeP;
				rNameP = "R_N" + nodeP;
				unique_push(rowNames, rNameP);
			...
			if (nodeP != "0" && nodeP.find("GND") == std::string::npos) {
				rNameP = "R_N" + nodeP;
				unique_push(rowNames, rNameP);
				bMatrixConductanceMap[rNameP][label] = label;
				pGND = false;
			}
			else pGND = true;
			if (nodeN != "0" && nodeN.find("GND") == std::string::npos) {
				rNameN = "R_N" + nodeN;
				unique_push(rowNames, rNameN);
				bMatrixConductanceMap[rNameN][label] = label;
				nGND = false;
			}
			else nGND = true;
		}
			...
			unique_push(rowNames, rName);
			unique_push(columnNames, cName);
			bMatrixConductanceMap[rName][label + "-PHASE"] = cName;
			if (nodeP != "0" && nodeP.find("GND") == std::string::npos) {
				cNameP = "C_NV" + nodeP;
				rNameP = "R_N" + nodeP;
				unique_push(rowNames, rNameP);
				unique_push(columnNames, cNameP);
				bMatrixConductanceMap[rNameP][label + "-CAP"] = std::to_string(jj_cap);
				bMatrixConductanceMap[rNameP][label + "-ICRIT"] = std::to_string(jj_icrit);
				pGND = false;
			}
			else pGND = true;
			if (nodeN != "0" && nodeN.find("GND") == std::string::npos) {
			...
			if (!nGND) {
				bMatrixConductanceMap[rName][label + "-V"] = cNameP + "-GND";
				bMatrixConductanceMap[rNameP][label + "-V"] = cNameP + "-GND";
			}
			else if (!pGND) {
				bMatrixConductanceMap[rName][label + "-V"] = "GND-" + cNameN;
				bMatrixConductanceMap[rNameN][label + "-V"] = "GND-" + cNameN;
			}
			else {
				bMatrixConductanceMap[rName][label + "-V"] = cNameP + "-" + cNameN;
				bMatrixConductanceMap[rNameP][label + "-V"] = cNameP + "-" + cNameN;
				bMatrixConductanceMap[rNameN][label + "-V"] = cNameP + "-" + cNameN;
			}
			matrix_element e;
