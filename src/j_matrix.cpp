...
matrix_A(InputFile& iFile)
{
  ...
  if (rowNames.size() != columnNames.size())
    matrix_errors(NON_SQUARE,
                  std::to_string(columnNames.size()) + "x" +
                    std::to_string(rowNames.size()));
  if (VERBOSE)
    std::cout << std::setw(35) << std::left
              << "A matrix dimensions: " << columnNames.size() << "\n\n";
}
/*
...
create_A_matrix(InputFile& iFile)
{
  ...
  std::vector<std::vector<double>> A_matrix(
    rowNames.size(), std::vector<double>(columnNames.size()));
