...
bool PJJ::update_value(const double& v) {
...
void PJJ::update_timestep(const double& factor) {
  h_ = h_ * factor;
  if (state_ == 0) {
    matrixInfo.nonZeros_.back() = -1 / subgap_impedance();
  } else if (state_ == 1) {
