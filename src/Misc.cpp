...
bool        Misc::isclose(const double& a, const double& b) { return fabs(a - b) < DBL_EPSILON; }
...
std::string Misc::vector_to_string(const tokens_t& s, std::string d) {
    std::stringstream ss;
    for (const auto& i : s) { ss << i << d; }
    return ss.str();
}

tokens_t Misc::tokenize(const std::string& c, std::string d, bool trimEmpty, bool trimSpaces, int64_t count) {
...
double Misc::modifier(const std::string& value) {
    ...
    } catch (std::exception&) { Errors::misc_errors(MiscErrors::STOD_ERROR, value); }
    switch (value.substr(sz)[0]) {
        ...
        case 'T':
            return number * 1E12;
            /* auto modifier */
        case 'E': return std::stod(value);
        default: return number;
    }
}

void Misc::unique_push(std::vector<std::string>& vector, const std::string& string) {
    if (std::find(vector.begin(), vector.end(), string) == vector.end()) { vector.push_back(string); }
}

int64_t Misc::index_of(const std::vector<std::string>& vector, const std::string& value) {
    int64_t counter = 0;
...
std::string Misc::substring_after(const std::string& str, const std::string& whatpart) {
    std::size_t pos = 0;
...
std::string Misc::substring_before(const std::string& str, const std::string& whatpart) {
    std::string substring;
...
bool Misc::findX(const std::vector<std::string>& segment, std::string& theLine, int64_t& linePos) {
    for (auto i = static_cast<size_t>(linePos); i < segment.size(); ++i) {
