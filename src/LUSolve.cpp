...
void LUSolve::create_matrix(int64_t                 shape,
                            std::vector<double>&    nz,
                            std::vector<long long>& ci,
                            std::vector<long long>& rp) {
    bool do_allocation = (allocated == false || m != shape || n != shape || nnz != nz.size());
    m = n = shape;
    nnz   = nz.size();
    if (do_allocation) {
        dCreate_CompCol_Matrix(&A, m, n, nnz, &nz.front(), &ci.front(), &rp.front(), SLU_NC, SLU_D, SLU_GE);
...
bool LUSolve::is_stable() { return ((info > 0 || rcond < 1e-8 || rpg > 1e8) == false); }
...
void LUSolve::solve(std::vector<double>& x) {
    if (!constructed) { ABORT("Preconditioner not constructed."); }
    DNformat    LHSstore = {static_cast<int_t>(x.size()), &x.front()};
    SuperMatrix LHSmat   = {SLU_DN, SLU_D, SLU_GE, static_cast<int_t>(x.size()), 1, &LHSstore};
