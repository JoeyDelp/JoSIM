...
double Parser::parse_param(
  ...
  while (!expToEval.empty()) {
    ...
    if (isdigit(partToEval[0])) {
      ...
      qType.push_back('V');
    } else if (parsedParams.count(JoSIM::ParameterName(partToEval, subckt)) !=
               0) {
...
int Parser::prec_lvl(const std::string &op) {
  switch (op[0]) {
...
double Parser::parse_operator(const std::string &op, double val1, double val2,
                              int &popCount) {
