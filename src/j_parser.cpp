...
std::string funcsArray[] = {"COS", "SIN", "TAN", "ACOS", "ASIN", "ATAN", "COSH", "SINH", "TANH", "ACOSH", "ASINH", "ATANH", "EXP",
                            "LOG", "LOG10", "SQRT", "CBRT"};
...
void parse_expression(std::string expName, std::string expr, std::string subckt) {
    ...
    while(expToEval != "") {
        ...
        if (opLoc == 0) partToEval = expToEval.substr(0, opLoc + 1);
        else partToEval = expToEval.substr(0, opLoc);
        ...
        // Else if operator
           ...
           /* While (((Function at top of operator stack) OR (Operator at top of operator stack with greater precedence)
        ...
        else parsing_errors(UNIDENTIFIED_PART, partToEval);
        // If operator location is zero substring after the operator
        if (opLoc == 0) expToEval = expToEval.substr(opLoc + 1);
        if (opLoc == -1) expToEval = "";
    ...
    // Now that we have the RPN notation for the expression we can solve it and store it
    while(rpnQueue.size() > 1) {
        ...
        for(int i = 0; i < qType.size(); i++) {
            ...
            // Else if an operator is identified
                ...
                if (i == 0) parsing_errors(INVALID_RPN, expr);
                // Else if i is less than 2 (meaning only 1 value identified)
                else if(i < 2) {
                    rpnQueueCopy.pop_back();
                    rpnQueueCopy.push_back(precise_to_string(parse_operator(rpnQueue[i], 0, modifier(rpnQueue[i-1]), popCount)));
                }
                // Else 2 or more values identified
                else {
                    result = parse_operator(rpnQueue[i], modifier(rpnQueue[i-2 ]), modifier(rpnQueue[i-1]), popCount);
                    for (int k = 0; k < popCount; k++) rpnQueueCopy.pop_back();
                    if (popCount == 2) qTypeCopy.pop_back();
...
double parse_operator(std::string op, double val1, double val2, int& popCount) {
    ...
    if(std::find(funcs.begin(), funcs.end(), op) != funcs.end()) {
        ...
        else if (op == "COS") return cos(val2);
        else if (op == "TAN") return tan(val2);
        else if (op == "ASIN") return asin(val2);
        else if (op == "ACOS") return acos(val2);
        else if (op == "ATAN") return atan(val2);
        else if (op == "SINH") return sinh(val2);
        else if (op == "COSH") return cosh(val2);
        else if (op == "TANH") return tanh(val2);
        else if (op == "ASINH") return asinh(val2);
        else if (op == "ACOSH") return acosh(val2);
        else if (op == "ATANH") return atanh(val2);
        else if (op == "EXP") return exp(val2);
        else if (op == "LOG") return log(val2);
        else if (op == "SQRT") return sqrt(val2);
        else if (op == "CBRT") return cbrt(val2);
    }
    else {
        popCount = 2;
        switch(op[0]) {
