    ...
    if(len(sys.argv) < 2):
        ...
        for f in ex_files:
            ...
            if(simulation == 0):
                csvfile = pre + '.csv'
                if("_wr" in file):
                    cmd = '{0} -c 1 -o {1} {2} -d 1 {3}'.format(josimpath, csvfile, file, nullchar)
                else:
                    cmd = '{0} -o {1} {2} -d 1 {3}'.format(josimpath, csvfile, file, nullchar)
                ...
                if("_wr" in file):
                    cmd = '{0} -c 1 -o {1} {2} -a 1 -d 1 {3}'.format(josimpath, csvfile, file, nullchar)
                else:
                    cmd = '{0} -o {1} {2} -a 1 -d 1 {3}'.format(josimpath, csvfile, file, nullchar)
                returned_value = os.system(cmd)
                if(returned_value == 0):
                    counter = counter + 1
                    print('Examples passed: {0}/{1}'.format(counter, len(ex_files)), end='\r')
