...
enum class ModelErrors : int64_t { PARAM_TYPE_ERROR, PARAM_PARENTHESIS, UNKNOWN_MODEL_TYPE, BAD_MODEL_DEFINITION };
...
class Errors {
  public:
    ...
    static void input_errors(InputErrors errorCode, string_o message = std::nullopt);

    static void invalid_component_errors(ComponentErrors errorCode, string_o componentLabel = std::nullopt);

    static void control_errors(ControlErrors errorCode, string_o message = std::nullopt);

    static void model_errors(ModelErrors errorCode, string_o message = std::nullopt);

    [[noreturn]] static void matrix_errors(MatrixErrors errorCode, string_o message = std::nullopt);

    [[noreturn]] static void misc_errors(MiscErrors errorCode, string_o message = std::nullopt);

    static void              function_errors(FunctionErrors errorCode, string_o message = std::nullopt);

    [[noreturn]] static void simulation_errors(SimulationErrors errorCode, string_o message = std::nullopt);

    static void              parsing_errors(ParsingErrors errorCode, string_o message = std::nullopt);

    static void              output_errors(OutputErrors errorCode, string_o message = std::nullopt);

    [[noreturn]] static void netlist_errors(NetlistErrors errorCode, string_o message = std::nullopt);

    static void              verbosity_errors(VerbosityErrors errorCode, string_o message = std::nullopt);
