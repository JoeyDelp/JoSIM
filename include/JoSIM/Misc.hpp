...
namespace Misc {
    ...
    int64_t     index_of(const std::vector<std::string>& vector, const std::string& value);

    std::string substring_after(const std::string& str, const std::string& whatpart);

    std::string substring_before(const std::string& str, const std::string& whatpart);

    bool        findX(const std::vector<std::string>& segment, std::string& theLine, int64_t& linePos);

    template <typename A, typename B> std::pair<B, A> flip_pair(const std::pair<A, B>& p) {
        return std::pair<B, A>(p.second, p.first);
    }

    template <typename A, typename B> std::map<B, A> flip_map(const std::unordered_map<A, B>& src) {
        std::map<B, A> dst;
        std::transform(src.begin(), src.end(), std::inserter(dst, dst.begin()), flip_pair<A, B>);
        return dst;
    }

    template <typename T> std::string precise_to_string(const T a_value) {
        std::ostringstream out;
        out << std::uppercase << std::setprecision(std::numeric_limits<long double>::digits10 + 1) << a_value;
        return out.str();
    }
