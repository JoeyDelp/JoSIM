...
inline void tight_layout(const double pad = 0.7, const double h_pad = 0.3, const double w_pad = 0.5) {
    PyObject* args = PyTuple_New(3);
    PyTuple_SetItem(args, 0, PyFloat_FromDouble(pad));
    PyTuple_SetItem(args, 1, PyFloat_FromDouble(h_pad));
    PyTuple_SetItem(args, 2, PyFloat_FromDouble(w_pad));

    PyObject *res = PyObject_CallObject(
        detail::_interpreter::get().s_python_function_tight_layout,
        args); //, detail::_interpreter::get().s_python_empty_tuple);
...
inline void suptitle(const std::string &titlestr)
{
    PyObject* pytitlestr = PyString_FromString(titlestr.c_str());
    PyObject* args = PyTuple_New(1);
    PyTuple_SetItem(args, 0, pytitlestr);
