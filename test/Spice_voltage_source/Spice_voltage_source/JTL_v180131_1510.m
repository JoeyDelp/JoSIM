...
sfq_down = 2.5E-12;
sfq_ampl = 600E-6;
IA = zeros(1, sfq_start/hn-1);
for i = (sfq_start/hn):((sfq_start + sfq_up)/hn-1)
    IA = [IA, sfq_ampl/sfq_up * (i*hn - sfq_start)];
end
for i = ((sfq_start + sfq_up)/hn):((sfq_start + sfq_up + sfq_down)/hn - 1)
    IA = [IA, 600E-6 - sfq_ampl/sfq_down * (i*hn - (sfq_start + sfq_up))];
end
[IA_rows,IA_cols] = size(IA);
IA = [IA, zeros(1, N-IA_cols)];
